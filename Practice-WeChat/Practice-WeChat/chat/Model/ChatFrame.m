...
- (void)setModel:(ChatModel *)model
{
    ...
    if (model.isSender) {
        ...
        if ([model.message.type isEqualToString:TypeText]) {
            ...
            UIImage *image   = [UIImage imageWithContentsOfFile:[[MediaManager sharedManager] imagePathWithName:model.mediaPath.lastPathComponent]];
            if (image) {
        ...
        } else if ([model.message.type isEqualToString:TypeVoice]) {
            // 语音消息
        ...
        }  else if ([model.message.type isEqualToString:TypeVideo]) {
            // 视频信息
            CGSize imageSize       = CGSizeMake(150, 150);
            UIImage *videoImage = [[MediaManager sharedManager] videoImageWithFileName:model.mediaPath.lastPathComponent];
            if (!videoImage) {
                //取第一针的图片
                NSString *path = [[VideoManager shareManager] receiveVideoPathWithFileKey:[model.mediaPath.lastPathComponent stringByDeletingPathExtension]];
        ...
        if ([model.message.type isEqualToString:TypeText]) {
            ...
            _bubbleViewF = CGRectMake(CGRectGetMaxX(_headImageViewF) + headToBubble, cellMargin+topViewH, 100, 40);
            ...
            // 假设
            _durationLabelF = CGRectMake(CGRectGetMaxX(_bubbleViewF) - voiceToBull - 50, cellMargin + 10 + topViewH, 50, 20);
            _redViewF = CGRectMake(CGRectGetMaxX(_bubbleViewF) + 6, CGRectGetMinY(_bubbleViewF) + _bubbleViewF.size.height*0.5-4, 8, 8);
            
        } else if ([model.message.type isEqualToString:TypeVideo]) {
            // 视频
            CGSize imageSize       = CGSizeMake(150, 150);
            UIImage *videoImage = [[MediaManager sharedManager] videoImageWithFileName:[NSString stringWithFormat:@"%@.png",model.message.fileKey]];
            if (!videoImage) {
                NSString *path = [[VideoManager shareManager] receiveVideoPathWithFileKey:model.message.fileKey];
                videoImage    = [UIImage videoFramerateWithPath:path];
            }
            if (videoImage) {
                float scale = videoImage.size.height/videoImage.size.width;
